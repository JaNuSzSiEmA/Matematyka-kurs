...
function getKeyForExercise(ex, keyObj) {
  ...
  return { kind: 'numeric', value: keyObj?.value ?? '' };
}

// ─── HintsEditor ──────────────────────────────────────────────────────────────

function HintsEditor({ hints, onChange }) {
  function addHint() { onChange([...hints, '']); }
  function updateHint(idx, value) { onChange(hints.map((h, i) => (i === idx ? value : h))); }
  function removeHint(idx) { onChange(hints.filter((_, i) => i !== idx)); }
...
function hintsFromArray(arr) {
  if (!Array.isArray(arr)) return null;
  const lines = arr.map((h) => String(h ?? '').trim()).filter(Boolean);
...
function parseTSV(text) {
  ...
  const hasHeader = first.some((h) =>
    ['title', 'prompt', 'answer_type', 'a', 'b', 'c', 'd', 'correct', 'numeric_value',
     'points_max', 'difficulty', 'status', 'description', 'image_url', 'solution_video_url', 'hints']
      .includes(String(h || '').toLowerCase())
  );
  let header = null;
  let startIdx = 0;
  if (hasHeader) { header = first.map((h) => String(h || '').toLowerCase()); startIdx = 1; }
  const rows = [];
  for (let i = startIdx; i < lines.length; i++) {
    ...
    if (!header) {
      ...
      rows.push({ __line: lineNo, prompt: prompt ?? '', answer_type: 'abcd', A: A ?? '', B: B ?? '', C: C ?? '', D: D ?? '', correct: correct ?? '', points_max: points_max ?? '', difficulty: difficulty ?? '', status: status ?? '' });
      continue;
    }
...
function normalizeBulkRow(raw) {
  ...
  const hints = raw.hints ? (Array.isArray(raw.hints) ? raw.hints : String(raw.hints).split('\n').map((l) => l.trim()).filter(Boolean)) : null;
  ...
  if (answer_type === 'abcd') {
    ...
    return { ok: true, value: { title, answer_type: 'abcd', prompt, points_max, difficulty, status, description, image_url, solution_video_url, hints, answer_key: { options: { A, B, C, D }, correct } } };
  }
  ...
  if (answer_type === 'numeric') {
    ...
    return { ok: true, value: { title, answer_type: 'numeric', prompt, points_max, difficulty, status, description, image_url, solution_video_url, hints, answer_key: { value: v } } };
  }
...
export default function AdminExerciseBank() {
  ...
  const [filters, setFilters] = useState({
    course_id: '', topic_section_id: '', status: 'all', answer_type: 'all', q: '',
    use_in_course: false, use_in_repertory: false, use_in_generator: false, use_in_minigame: false,
  });

  // ── Create form — hints jako tablica ──
  const [create, setCreate] = useState({
    title: '', course_id: '', topic_section_id: '', prompt: '', description: '', image_url: '',
    answer_type: 'abcd', optionsA: '', optionsB: '', optionsC: '', optionsD: '',
    correct_choice: 'A', correct_numeric: '', points_max: 1, difficulty: 1,
    requires_ai: false, requires_photo: false, status: 'draft', solution_video_url: '',
    hints: [],   // ← tablica zamiast hints_text
    use_in_course: false, use_in_repertory: true, use_in_generator: true, use_in_minigame: true,
  });
  ...
  const [bulk, setBulk] = useState({
    mode: 'tsv', course_id: '', topic_section_id: '', status: 'draft', points_max: 1, difficulty: 1,
    use_in_course: false, use_in_repertory: true, use_in_generator: true, use_in_minigame: true,
    requires_ai: false, requires_photo: false, text: '',
  });
  ...
  async function load() {
    ...
    const { data: courseRows, error: courseErr } = await supabase.from('courses').select('id, title, description').order('title', { ascending: true });
    if (courseErr) { setMsg(`Load courses failed: ${courseErr.message}`); setCourses([]); setLoading(false); return; }
    ...
    if (!bulk.course_id && defaultCourseId) setBulk((p) => ({ ...p, course_id: defaultCourseId }));

    const courseForSections = defaultCourseId || filters.course_id;
    if (courseForSections) {
      const { data: secRows, error: secErr } = await supabase.from('sections').select('id, title, slug, order_index, course_id').eq('course_id', String(courseForSections)).order('order_index', { ascending: true });
      if (secErr) { setMsg(`Load sections failed: ${secErr.message}`); setSections([]); }
      else {
        setSections(secRows || []);
        if (!create.topic_section_id && (secRows || []).length > 0) setCreate((p) => ({ ...p, topic_section_id: secRows[0].id }));
        if (!bulk.topic_section_id && (secRows || []).length > 0) setBulk((p) => ({ ...p, topic_section_id: secRows[0].id }));
      }
    } else { setSections([]); }

    const { data: exData, error: exErr } = await supabase.from('exercises')
      .select('id, title, course_id, topic_section_id, prompt, answer_type, points_max, difficulty, requires_ai, requires_photo, image_url, description, hints, solution_video_url, status, use_in_course, use_in_repertory, use_in_generator, use_in_minigame, created_at')
      .order('created_at', { ascending: false }).limit(500);

    if (exErr) { setMsg(`Load exercises failed: ${exErr.message}`); setExercises([]); setAnswerKeysByExerciseId({}); setEditById({}); setLoading(false); return; }

    const ids = (exData || []).map((x) => x.id);
    let keyMap = {};
    if (ids.length > 0) {
      const { data: keyRows, error: keyErr } = await supabase.from('exercise_answer_keys').select('exercise_id, answer_key').in('exercise_id', ids);
      if (keyErr) setMsg(`Load answer keys failed: ${keyErr.message}`);
      else keyMap = Object.fromEntries((keyRows || []).map((k) => [k.exercise_id, k.answer_key]));
    }

    setExercises(exData || []);
    setAnswerKeysByExerciseId(keyMap);

    // init editor — hints jako tablica
    const nextEdit = {};
    for (const e of exData || []) {
      ...
      nextEdit[e.id] = {
        ...
        hints: Array.isArray(e.hints) ? e.hints : [],   // ← tablica
  ...
  useEffect(() => {
    (async () => {
      ...
      const { data: secRows, error: secErr } = await supabase.from('sections').select('id, title, slug, order_index, course_id').eq('course_id', String(filters.course_id)).order('order_index', { ascending: true });
      if (secErr) { setMsg(`Load sections failed: ${secErr.message}`); setSections([]); return; }
      setSections(secRows || []);
  ...
  const filtered = useMemo(() => {
    ...
    return exercises.filter((e) => {
      ...
      if (term) { const hay = `${e.title || ''} ${e.prompt || ''}`.toLowerCase(); if (!hay.includes(term)) return false; }
      return true;
    });
  }, [exercises, filters]);
  ...
  function buildAnswerKeyFromCreate() {
    if (create.answer_type === 'abcd') {
      ...
      const options = { A: String(create.optionsA || '').trim(), B: String(create.optionsB || '').trim(), C: String(create.optionsC || '').trim(), D: String(create.optionsD || '').trim() };
      for (const k of ['A', 'B', 'C', 'D']) { if (!options[k]) return { error: `Uzupełnij treść odpowiedzi ${k}.` }; }
  ...
  async function createExercise() {
    setMsg('');
    if (!create.course_id) { setMsg('Wybierz course.'); return; }
    ...
    const hints = hintsFromArray(create.hints);

    const { data: inserted, error: insErr } = await supabase.from('exercises').insert({
    ...
    const { error: keyErr } = await supabase.from('exercise_answer_keys').insert({ exercise_id: inserted.id, answer_key: answerKey.value });
    if (keyErr) { setMsg(`Exercise created, but answer key failed: ${keyErr.message}`); await load(); return; }

    setCreate((p) => ({
      ...p, title: '', prompt: '', description: '', image_url: '', solution_video_url: '',
      hints: [],   // ← reset tablicy
      points_max: 1, difficulty: 1, requires_ai: false, requires_photo: false, status: 'draft',
      answer_type: 'abcd', correct_choice: 'A', correct_numeric: '',
  ...
  function getDirty(id) {
    ...
    // porównujemy hints jako JSON
    const hintsNow = JSON.stringify(Array.isArray(e.hints) ? e.hints : []);
    const hintsEd = JSON.stringify(Array.isArray(ed.hints) ? ed.hints : []);

    if (
      String(e.title ?? '') !== String(ed.title ?? '') ||
    ...
    if (ed.answer_type === 'abcd') {
      ...
      const curOpts = keyParsed.kind === 'abcd' ? { A: keyParsed.A, B: keyParsed.B, C: keyParsed.C, D: keyParsed.D } : { A: '', B: '', C: '', D: '' };
      if (normalizeChoice(ed.correct_choice) !== normalizeChoice(curCorrect)) return true;
      if (String(ed.optionsA || '') !== String(curOpts.A || '') || String(ed.optionsB || '') !== String(curOpts.B || '') || String(ed.optionsC || '') !== String(curOpts.C || '') || String(ed.optionsD || '') !== String(curOpts.D || '')) return true;
    } else {
  ...
  async function saveExercise(id) {
    ...
    if (ed.answer_type === 'abcd') {
      const options = { A: String(ed.optionsA || '').trim(), B: String(ed.optionsB || '').trim(), C: String(ed.optionsC || '').trim(), D: String(ed.optionsD || '').trim() };
      for (const k of ['A', 'B', 'C', 'D']) { if (!options[k]) { setMsg(`Exercise ${id}: brak treści odpowiedzi ${k}.`); return; } }
      nextAnswerKey = { options, correct: normalizeChoice(ed.correct_choice || 'A') };
    } else {
      const v = parseNumericValue(ed.correct_numeric);
      if (v === null) { setMsg(`Exercise ${id}: brak poprawnej odpowiedzi numeric.`); return; }
      nextAnswerKey = { value: v };
    }

    setSavingById((p) => ({ ...p, [id]: true }));
    try {
      const hints = hintsFromArray(ed.hints);

      const { error: updErr } = await supabase.from('exercises').update({
        title: ed.title || null,
      ...
      const { error: keyErr } = await supabase.from('exercise_answer_keys').upsert({ exercise_id: id, answer_key: nextAnswerKey }, { onConflict: 'exercise_id' });
      if (keyErr) { setMsg(keyErr.message); return; }
  ...
  async function deleteExercise(id) {
    setMsg('');
    if (!window.confirm('Usunąć exercise? Uwaga: island_items mogą się zepsuć.')) return;
  ...
  useEffect(() => {
    ...
    if (!text.trim()) { setBulkPreview({ rows: [], errors: [] }); return; }
    if (bulk.mode === 'tsv') {
      ...
      if (!parsed.ok) { setBulkPreview({ rows: [], errors: [{ line: 0, error: parsed.error }] }); return; }
      if (!Array.isArray(parsed.value)) { setBulkPreview({ rows: [], errors: [{ line: 0, error: 'JSON must be an array' }] }); return; }
      const preview = [], errors = [];
  ...
  async function runBulkImport() {
    setMsg('');
    if (!bulk.course_id) { setMsg('Bulk: wybierz course.'); return; }
    if (!bulkPreview.rows.length) { setMsg('Bulk: brak wierszy.'); return; }
    if (bulkPreview.errors.length) { setMsg('Bulk: popraw błędy.'); return; }
    if (!window.confirm(`Zaimportować ${bulkPreview.rows.length} ćwiczeń?`)) return;
    setBulkImporting(true);
    const errors = [];
    try {
      for (const row of bulkPreview.rows) {
        const rawOverrides = bulk.mode === 'json' ? row.__raw || {} : {};
        const points_max = rawOverrides.points_max !== undefined ? Number(rawOverrides.points_max) : row.points_max ?? Number(bulk.points_max);
        const difficulty = rawOverrides.difficulty !== undefined ? Number(rawOverrides.difficulty) : row.difficulty ?? Number(bulk.difficulty);
        const status = rawOverrides.status !== undefined ? String(rawOverrides.status) : row.status || bulk.status;
        const use_in_course = rawOverrides.use_in_course !== undefined ? Boolean(rawOverrides.use_in_course) : Boolean(bulk.use_in_course);
        const use_in_repertory = rawOverrides.use_in_repertory !== undefined ? Boolean(rawOverrides.use_in_repertory) : Boolean(bulk.use_in_repertory);
        const use_in_generator = rawOverrides.use_in_generator !== undefined ? Boolean(rawOverrides.use_in_generator) : Boolean(bulk.use_in_generator);
        const use_in_minigame = rawOverrides.use_in_minigame !== undefined ? Boolean(rawOverrides.use_in_minigame) : Boolean(bulk.use_in_minigame);
        const requires_ai = rawOverrides.requires_ai !== undefined ? Boolean(rawOverrides.requires_ai) : Boolean(bulk.requires_ai);
        const requires_photo = rawOverrides.requires_photo !== undefined ? Boolean(rawOverrides.requires_photo) : Boolean(bulk.requires_photo);
        const topic_section_id = rawOverrides.topic_section_id !== undefined ? String(rawOverrides.topic_section_id || '') : String(bulk.topic_section_id || '');
        const title = rawOverrides.title !== undefined ? String(rawOverrides.title || '') : String(row.title || '');
        const { data: inserted, error: insErr } = await supabase.from('exercises').insert({
          course_id: String(bulk.course_id), topic_section_id: topic_section_id ? String(topic_section_id) : null,
          title: title ? title : null, prompt: row.prompt, answer_type: row.answer_type,
          points_max: Number(points_max), difficulty: Number(difficulty), status,
          description: row.description || null, image_url: row.image_url || null,
          solution_video_url: row.solution_video_url || null, hints: row.hints || null,
          use_in_course, use_in_repertory, use_in_generator, use_in_minigame, requires_ai, requires_photo,
        }).select('id').single();
        if (insErr) { errors.push({ line: row.line, error: insErr.message }); continue; }
        const { error: keyErr } = await supabase.from('exercise_answer_keys').insert({ exercise_id: inserted.id, answer_key: row.answer_key });
        if (keyErr) { errors.push({ line: row.line, error: `answer_key failed: ${keyErr.message}` }); continue; }
      }
    } finally { setBulkImporting(false); }
    if (errors.length) setMsg(`Bulk errors. First: line ${errors[0].line}: ${errors[0].error}`);
    else { setMsg(`Bulk import OK: ${bulkPreview.rows.length} ćwiczeń.`); setBulk((p) => ({ ...p, text: '' })); }
    await load();
  }
      ...
      <div className="min-h-screen bg-white">
        <div className="mx-auto max-w-7xl p-6">
          <div className="flex items-center justify-between gap-4">
            <div>
              <Link href="/admin" className="text-sm font-semibold text-gray-700 underline">← Admin</Link>
            ...
            <div className="rounded-2xl border border-gray-200 p-4">
              ...
              <label className="mt-3 block">
                ...
                <input className="mt-1 w-full rounded-xl border border-gray-300 px-3 py-2 text-sm"
                  ...
                  value={create.title} onChange={(e) => setCreate((p) => ({ ...p, title: e.target.value }))} />
              </label>
              ...
              <div className="mt-3 grid gap-3 sm:grid-cols-2">
                <label>
                  ...
                  <select className="mt-1 w-full rounded-xl border border-gray-300 px-3 py-2 text-sm"
                    value={create.course_id} onChange={(e) => setCreate((p) => ({ ...p, course_id: e.target.value }))}>
                    <option value="">Wybierz…</option>
                    {courses.map((c) => <option key={c.id} value={String(c.id)}>{getCourseLabel(c)}</option>)}
                  </select>
                </label>
                <label>
                  <div className="text-xs font-semibold text-gray-600">Topic (sekcja)</div>
                  <select className="mt-1 w-full rounded-xl border border-gray-300 px-3 py-2 text-sm"
                    value={create.topic_section_id || ''} onChange={(e) => setCreate((p) => ({ ...p, topic_section_id: e.target.value }))}>
                    <option value="">(brak)</option>
                    {sections.map((s) => <option key={s.id} value={s.id}>{s.order_index}. {s.title}</option>)}
                  </select>
                </label>
              </div>
              ...
              <label className="mt-3 block">
                ...
                <textarea ref={createPromptRef} className="mt-1 min-h-[120px] w-full rounded-xl border border-gray-300 px-3 py-2 text-sm"
                  value={create.prompt} onChange={(e) => setCreate((p) => ({ ...p, prompt: e.target.value }))} />
              </label>
              <MathKeyboard onInsert={insertIntoCreatePrompt} />

              <label className="mt-3 block">
                <div className="text-xs font-semibold text-gray-600">description (optional)</div>
                <textarea className="mt-1 min-h-[70px] w-full rounded-xl border border-gray-300 px-3 py-2 text-sm"
                  value={create.description} onChange={(e) => setCreate((p) => ({ ...p, description: e.target.value }))} />
              </label>
              ...
              <div className="mt-3 grid gap-3 sm:grid-cols-2">
                <label>
                  ...
                  <select className="mt-1 w-full rounded-xl border border-gray-300 px-3 py-2 text-sm"
                    value={create.answer_type} onChange={(e) => setCreate((p) => ({ ...p, answer_type: e.target.value }))}>
                    <option value="abcd">abcd</option><option value="numeric">numeric</option>
                  </select>
                </label>
                <label>
                  <div className="text-xs font-semibold text-gray-600">status</div>
                  <select className="mt-1 w-full rounded-xl border border-gray-300 px-3 py-2 text-sm"
                    value={create.status} onChange={(e) => setCreate((p) => ({ ...p, status: e.target.value }))}>
                ...
                <label>
                  ...
                  <input type="number" className="mt-1 w-full rounded-xl border border-gray-300 px-3 py-2 text-sm"
                    value={create.points_max} onChange={(e) => setCreate((p) => ({ ...p, points_max: e.target.value }))} />
                </label>
                <label>
                  <div className="text-xs font-semibold text-gray-600">difficulty</div>
                  <input type="number" className="mt-1 w-full rounded-xl border border-gray-300 px-3 py-2 text-sm"
                    value={create.difficulty} onChange={(e) => setCreate((p) => ({ ...p, difficulty: e.target.value }))} />
                </label>
              </div>
                  ...
                  <div className="mt-2 grid gap-2">
                    {['A', 'B', 'C', 'D'].map((opt) => (
                      <div key={opt} className="grid grid-cols-[40px_1fr_110px] items-center gap-2">
                        ...
                        <input ref={(el) => { createOptionRefs.current[opt] = el; }}
                          className="w-full rounded-xl border border-gray-300 px-3 py-2 text-sm"
                          placeholder={`Treść odpowiedzi ${opt}`}
                          value={create[`options${opt}`]}
                          onFocus={() => setCreateActiveAnswer(opt)}
                          onChange={(e) => { const v = e.target.value; setCreate((p) => ({ ...p, [`options${opt}`]: v })); }} />
                        <button type="button"
                          className={['rounded-xl border px-3 py-2 text-sm font-semibold', create.correct_choice === opt ? 'border-gray-900 bg-gray-900 text-white' : 'border-gray-300 bg-white text-gray-900'].join(' ')}
                          onClick={() => setCreate((p) => ({ ...p, correct_choice: opt }))}>Poprawna</button>
                      </div>
                    ))}
                  </div>
                ) : (
                  <input ref={createNumericRef} className="mt-2 w-full rounded-xl border border-gray-300 px-3 py-2 text-sm"
                    placeholder="Poprawna odpowiedź (np. 12)" value={create.correct_numeric}
              ...
              <label className="mt-3 block">
                ...
                <input className="mt-1 w-full rounded-xl border border-gray-300 px-3 py-2 text-sm"
                  value={create.image_url} onChange={(e) => setCreate((p) => ({ ...p, image_url: e.target.value }))} />
              </label>

              <label className="mt-3 block">
                <div className="text-xs font-semibold text-gray-600">solution_video_url (optional)</div>
                <input className="mt-1 w-full rounded-xl border border-gray-300 px-3 py-2 text-sm"
                  value={create.solution_video_url} onChange={(e) => setCreate((p) => ({ ...p, solution_video_url: e.target.value }))} />
              </label>
              ...
              <div className="mt-3 rounded-xl border border-amber-100 bg-amber-50/40 p-3">
                ...
                <HintsEditor hints={create.hints} onChange={(next) => setCreate((p) => ({ ...p, hints: next }))} />
              </div>
              ...
              <div className="mt-3 grid gap-2">
                ...
                {['use_in_course', 'use_in_repertory', 'use_in_generator', 'use_in_minigame'].map((k) => (
                  <label key={k} className="flex items-center gap-2 text-sm text-gray-700">
                    <input type="checkbox" checked={Boolean(create[k])} onChange={(e) => setCreate((p) => ({ ...p, [k]: e.target.checked }))} />
                    {k}
                  </label>
                ))}
              </div>
              ...
              <div className="rounded-2xl border border-gray-200 p-4">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <select className="rounded-xl border border-gray-300 px-3 py-2 text-sm"
                    value={bulk.mode} onChange={(e) => setBulk((p) => ({ ...p, mode: e.target.value }))}>
                    <option value="tsv">TSV</option><option value="json">JSON</option>
                  </select>
                </div>
                <div className="mt-3 text-xs text-gray-600">Tip: you can include a <b>title</b> column in TSV header or JSON field.</div>
                <div className="mt-3 grid gap-3 sm:grid-cols-2">
                  <label>
                    <div className="text-xs font-semibold text-gray-600">course</div>
                    <select className="mt-1 w-full rounded-xl border border-gray-300 px-3 py-2 text-sm"
                      value={bulk.course_id} onChange={(e) => setBulk((p) => ({ ...p, course_id: e.target.value }))}>
                      <option value="">Wybierz…</option>
                      {courses.map((c) => <option key={c.id} value={String(c.id)}>{getCourseLabel(c)}</option>)}
                    </select>
                  </label>
                  <label>
                    ...
                    <select className="mt-1 w-full rounded-xl border border-gray-300 px-3 py-2 text-sm"
                      value={bulk.topic_section_id || ''} onChange={(e) => setBulk((p) => ({ ...p, topic_section_id: e.target.value }))}>
                      <option value="">(brak)</option>
                      {sections.map((s) => <option key={s.id} value={s.id}>{s.order_index}. {s.title}</option>)}
                    </select>
                  </label>
                  <label>
                    ...
                    <select className="mt-1 w-full rounded-xl border border-gray-300 px-3 py-2 text-sm"
                      value={bulk.status} onChange={(e) => setBulk((p) => ({ ...p, status: e.target.value }))}>
                      <option value="draft">draft</option><option value="published">published</option><option value="archived">archived</option>
                    </select>
                  </label>
                  <label>
                    ...
                    <input type="number" className="mt-1 w-full rounded-xl border border-gray-300 px-3 py-2 text-sm"
                      value={bulk.points_max} onChange={(e) => setBulk((p) => ({ ...p, points_max: e.target.value }))} />
                  </label>
                  <label>
                    <div className="text-xs font-semibold text-gray-600">default difficulty</div>
                    <input type="number" className="mt-1 w-full rounded-xl border border-gray-300 px-3 py-2 text-sm"
                      value={bulk.difficulty} onChange={(e) => setBulk((p) => ({ ...p, difficulty: e.target.value }))} />
                  </label>
                </div>
                <div className="mt-3">
                  ...
                  <div className="mt-2 grid grid-cols-2 gap-2 text-sm text-gray-700">
                    {['use_in_course', 'use_in_repertory', 'use_in_generator', 'use_in_minigame'].map((k) => (
                      <label key={k} className="flex items-center gap-2">
                        <input type="checkbox" checked={Boolean(bulk[k])} onChange={(e) => setBulk((p) => ({ ...p, [k]: e.target.checked }))} />{k}
                      </label>
                    ))}
                  </div>
                </div>
                ...
                <label className="mt-3 block">
                  <div className="text-xs font-semibold text-gray-600">{bulk.mode === 'tsv' ? 'Paste TSV' : 'Paste JSON array'}</div>
                  <textarea className="mt-1 min-h-[160px] w-full rounded-xl border border-gray-300 px-3 py-2 font-mono text-xs"
                    value={bulk.text} onChange={(e) => setBulk((p) => ({ ...p, text: e.target.value }))} />
                </label>
                <div className="mt-2 text-sm text-gray-700">Preview: <b>{bulkPreview.rows.length}</b> rows • Errors: <b>{bulkPreview.errors.length}</b></div>
                {bulkPreview.errors.length ? (
                  <div className="mt-2 rounded-xl border border-red-200 bg-red-50 p-3 text-xs text-red-700">
                    {bulkPreview.errors.slice(0, 8).map((e, idx) => <div key={idx}>line {e.line}: {e.error}</div>)}
                    {bulkPreview.errors.length > 8 ? <div>…</div> : null}
                  </div>
                ) : null}
                <button type="button" disabled={bulkImporting || !bulkPreview.rows.length || bulkPreview.errors.length}
              ...
              <div className="rounded-2xl border border-gray-200 p-4">
                ...
                <div className="mt-3 grid gap-3 sm:grid-cols-2">
                  <label>
                    <div className="text-xs font-semibold text-gray-600">Course</div>
                    <select className="mt-1 w-full rounded-xl border border-gray-300 px-3 py-2 text-sm"
                      value={filters.course_id} onChange={(e) => setFilters((p) => ({ ...p, course_id: e.target.value }))}>
                  ...
                  <label>
                    ...
                    <select className="mt-1 w-full rounded-xl border border-gray-300 px-3 py-2 text-sm"
                      value={filters.topic_section_id} disabled={!filters.course_id}
                  ...
                  <label>
                    <div className="text-xs font-semibold text-gray-600">answer_type</div>
                    <select className="mt-1 w-full rounded-xl border border-gray-300 px-3 py-2 text-sm"
                      value={filters.answer_type} onChange={(e) => setFilters((p) => ({ ...p, answer_type: e.target.value }))}>
                      <option value="all">(all)</option><option value="abcd">abcd</option><option value="numeric">numeric</option>
                    </select>
                  </label>
                  ...
                  <div className="sm:col-span-2">
                    <div className="text-xs font-semibold text-gray-600">Flag filters</div>
                    <div className="mt-2 grid grid-cols-2 gap-2 text-sm text-gray-700">
                      {['use_in_course', 'use_in_repertory', 'use_in_generator', 'use_in_minigame'].map((k) => (
                        <label key={k} className="flex items-center gap-2">
                          <input type="checkbox" checked={filters[k]} onChange={(e) => setFilters((p) => ({ ...p, [k]: e.target.checked }))} />{k}
                        </label>
                      ))}
                    </div>
                  </div>
                ...
                {loading ? <div className="mt-4 text-sm text-gray-700">Ładowanie…</div>
                  : filtered.length === 0 ? <div className="mt-4 text-sm text-gray-700">Brak wyników.</div>
                  : (
                    <div className="mt-4 space-y-4">
                      {filtered.map((e) => {
                        ...
                        return (
                          <div key={e.id} id={`exercise-card-${e.id}`}
                            className={['rounded-xl border p-3', focusedExerciseId === String(e.id) ? 'border-indigo-400 bg-indigo-50' : 'border-gray-200'].join(' ')}>

                            <div className="flex items-start justify-between gap-3">
                              <div>
                                <div className="text-sm font-semibold text-gray-900">
                                  {(ed.title || '(no title)') + ' • '}{ed.status} • {ed.answer_type} • {Number(ed.points_max)}pkt • diff {Number(ed.difficulty)}
                                  {dirty ? <span className="ml-2 text-xs text-orange-700">(unsaved)</span> : null}
                                </div>
                                <div className="mt-1 text-[11px] text-gray-500">id: <code>{e.id}</code></div>
                              </div>
                              <div className="flex items-center gap-2">
                                <button type="button" disabled={!dirty || saving} onClick={() => saveExercise(e.id)}
                                  className="rounded-lg border border-gray-900 bg-gray-900 px-3 py-1 text-sm font-semibold text-white disabled:opacity-50">
                                  {saving ? 'Saving…' : 'Save'}
                                </button>
                                <button type="button" onClick={() => deleteExercise(e.id)}
                            ...
                            <label className="mt-3 block">
                              ...
                              <input className="mt-1 w-full rounded-xl border border-gray-300 px-3 py-2 text-sm"
                                value={ed.title} placeholder="np. Funkcje kwadratowe — zadanie 1"
                                onChange={(ev) => setEditById((p) => ({ ...p, [e.id]: { ...p[e.id], title: ev.target.value } }))} />
                            </label>
                            ...
                            <label className="mt-3 block">
                              ...
                              <textarea className="mt-1 min-h-[90px] w-full rounded-xl border border-gray-300 px-3 py-2 text-sm"
                                value={ed.prompt} onChange={(ev) => setEditById((p) => ({ ...p, [e.id]: { ...p[e.id], prompt: ev.target.value } }))} />
                            </label>
                            ...
                            <label className="mt-3 block">
                              ...
                              <textarea className="mt-1 min-h-[60px] w-full rounded-xl border border-gray-300 px-3 py-2 text-sm"
                                value={ed.description} onChange={(ev) => setEditById((p) => ({ ...p, [e.id]: { ...p[e.id], description: ev.target.value } }))} />
                            </label>
                            ...
                            <div className="mt-3 grid gap-3 sm:grid-cols-2">
                              <label>
                                <div className="text-xs font-semibold text-gray-600">status</div>
                                <select className="mt-1 w-full rounded-xl border border-gray-300 px-3 py-2 text-sm"
                                  value={ed.status} onChange={(ev) => setEditById((p) => ({ ...p, [e.id]: { ...p[e.id], status: ev.target.value } }))}>
                                  <option value="draft">draft</option><option value="published">published</option><option value="archived">archived</option>
                                </select>
                              </label>
                              <label>
                                ...
                                <select className="mt-1 w-full rounded-xl border border-gray-300 px-3 py-2 text-sm"
                                  ...
                                  onChange={(ev) => setEditById((p) => ({ ...p, [e.id]: { ...p[e.id], answer_type: ev.target.value, correct_choice: 'A', optionsA: '', optionsB: '', optionsC: '', optionsD: '', correct_numeric: '' } }))}>
                                  <option value="abcd">abcd</option><option value="numeric">numeric</option>
                                </select>
                              </label>
                              <label>
                                <div className="text-xs font-semibold text-gray-600">points_max</div>
                                <input type="number" className="mt-1 w-full rounded-xl border border-gray-300 px-3 py-2 text-sm"
                                  value={ed.points_max} onChange={(ev) => setEditById((p) => ({ ...p, [e.id]: { ...p[e.id], points_max: ev.target.value } }))} />
                              </label>
                              <label>
                                <div className="text-xs font-semibold text-gray-600">difficulty</div>
                                <input type="number" className="mt-1 w-full rounded-xl border border-gray-300 px-3 py-2 text-sm"
                                  value={ed.difficulty} onChange={(ev) => setEditById((p) => ({ ...p, [e.id]: { ...p[e.id], difficulty: ev.target.value } }))} />
                              </label>
                              <label className="sm:col-span-2">
                                <div className="text-xs font-semibold text-gray-600">image_url</div>
                                <input className="mt-1 w-full rounded-xl border border-gray-300 px-3 py-2 text-sm"
                                  value={ed.image_url} onChange={(ev) => setEditById((p) => ({ ...p, [e.id]: { ...p[e.id], image_url: ev.target.value } }))} />
                              </label>
                              <label className="sm:col-span-2">
                                <div className="text-xs font-semibold text-gray-600">solution_video_url</div>
                                <input className="mt-1 w-full rounded-xl border border-gray-300 px-3 py-2 text-sm"
                                  value={ed.solution_video_url} onChange={(ev) => setEditById((p) => ({ ...p, [e.id]: { ...p[e.id], solution_video_url: ev.target.value } }))} />
                              </label>
                            </div>
                            ...
                            <div className="mt-3 rounded-xl border border-amber-100 bg-amber-50/40 p-3">
                              <div className="mb-2 text-sm font-semibold text-gray-900">
                                Podpowiedzi{' '}
                                <span className="font-normal text-gray-500">
                                  ({ed.hints.length} {ed.hints.length === 1 ? 'podpowiedź' : 'podpowiedzi'})
                                </span>
                              </div>
                              <HintsEditor
                                hints={ed.hints}
                                onChange={(next) => setEditById((p) => ({ ...p, [e.id]: { ...p[e.id], hints: next } }))}
                              />
                            </div>
                            ...
                            <div className="mt-3">
                              ...
                              <div className="mt-2 grid grid-cols-2 gap-2 text-sm text-gray-700">
                                {['use_in_course', 'use_in_repertory', 'use_in_generator', 'use_in_minigame'].map((k) => (
                                  <label key={k} className="flex items-center gap-2">
                                    <input type="checkbox" checked={Boolean(ed[k])} onChange={(ev) => setEditById((p) => ({ ...p, [e.id]: { ...p[e.id], [k]: ev.target.checked } }))} />{k}
                                  </label>
                                ))}
                              </div>
                            </div>
                                  ...
                                  {['A', 'B', 'C', 'D'].map((opt) => (
                                    <div key={opt} className="grid grid-cols-[40px_1fr_110px] items-center gap-2">
                                      ...
                                      <input className="w-full rounded-xl border border-gray-300 px-3 py-2 text-sm"
                                        value={ed[`options${opt}`]}
                                        onChange={(ev) => { const v = ev.target.value; setEditById((p) => ({ ...p, [e.id]: { ...p[e.id], [`options${opt}`]: v } })); }} />
                                      <button type="button"
                                        className={['rounded-xl border px-3 py-2 text-sm font-semibold', normalizeChoice(ed.correct_choice) === opt ? 'border-gray-900 bg-gray-900 text-white' : 'border-gray-300 bg-white text-gray-900'].join(' ')}
                                        onClick={() => setEditById((p) => ({ ...p, [e.id]: { ...p[e.id], correct_choice: opt } }))}>Poprawna</button>
                                    </div>
                                  ))}
                                </div>
                              ) : (
                                <input className="mt-2 w-full rounded-xl border border-gray-300 px-3 py-2 text-sm"
                                  placeholder="Poprawna odpowiedź" value={ed.correct_numeric}
                                  onChange={(ev) => setEditById((p) => ({ ...p, [e.id]: { ...p[e.id], correct_numeric: ev.target.value } }))} />
                              )}
                            </div>
                          </div>
                        );
